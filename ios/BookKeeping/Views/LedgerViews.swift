...
struct DashboardView: View { @State private var data: Dashboard?; private let month = String(Calendar.current.dateComponents([.year,.month], from: .now).year!) + "-" + String(format:"%02d",Calendar.current.component(.month,from:.now)); var body: some View { NavigationStack { ScrollView { if let data { VStack(alignment:.leading,spacing:16) { Text("本月账本").font(.largeTitle.bold()); HStack { AmountCard(title:"收入",amount:data.income,color:.green); AmountCard(title:"支出",amount:data.expense,color:.red) }; AmountCard(title:"结余",amount:data.balance,color:PixelPalette.leaf); Text("支出分类").font(.title2.bold()); ForEach(data.expenseBreakdown) { item in HStack { Image(systemName:item.icon); Text(item.name); Spacer(); Text(item.amount,format:.currency(code:"CNY")) } } }.padding() } else { ProgressView().task { data = try? await APIClient.shared.request("api/dashboard?month=\(month)") } } }.background(PixelPalette.paper) } }
struct AmountCard: View { let title:String;let amount:Decimal;let color:Color; var body: some View { VStack(alignment:.leading){Text(title).font(.caption.bold());Text(amount,format:.currency(code:"CNY")).font(.title2.bold())}.frame(maxWidth:.infinity,alignment:.leading).padding().background(.white).overlay(Rectangle().stroke(color,lineWidth:3)).shadow(color:color.opacity(0.35),x:3,y:3) } }
struct TransactionsView: View {
    @State private var rows = [LedgerTransaction](); @State private var error: String?
    private let month = Month.current
    var body: some View { NavigationStack { Group { if rows.isEmpty { ContentUnavailableView("暂无流水",systemImage:"list.bullet",description:Text("记下第一笔收支吧。")) } else { List(rows) { row in HStack { Image(systemName: row.categoryIcon).frame(width:28); VStack(alignment:.leading){Text(row.categoryName);Text(row.occurredOn).font(.caption).foregroundStyle(.secondary)}; Spacer(); Text(row.amount,format:.currency(code:"CNY")).foregroundStyle(row.type == .EXPENSE ? .red:.green) } } } }.navigationTitle("\(month) 流水").task { do { rows = try await APIClient.shared.request("api/transactions?month=\(month)") } catch { self.error=error.localizedDescription } }.overlay(alignment:.bottom) { if let error { Text(error).padding().foregroundStyle(.red) } } } }
}
