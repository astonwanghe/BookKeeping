...
public interface LedgerMapper {
    ...
    @Insert("INSERT INTO t_user(phone,password_hash) VALUES(#{phone},#{hash})")
    ...
    @Select("SELECT id,name,type,icon,sort_order AS sortOrder,active FROM t_category WHERE (user_id IS NULL OR user_id=#{userId}) AND active=TRUE ORDER BY type, sort_order, id")
    ...
    @Select("SELECT id,type FROM t_category WHERE id=#{categoryId} AND (user_id IS NULL OR user_id=#{userId}) AND active=TRUE")
    Map<String, Object> ownedCategory(long userId, long categoryId);

    @Select("SELECT t.id,t.type,t.amount,t.occurred_on AS occurredOn,t.note,t.category_id AS categoryId,c.name AS categoryName,c.icon AS categoryIcon FROM t_transaction t JOIN t_category c ON c.id=t.category_id WHERE t.user_id=#{userId} AND t.occurred_on BETWEEN #{from} AND #{to} ORDER BY t.occurred_on DESC,t.id DESC")
    ...
    @Select("SELECT id,category_id AS categoryId,amount FROM t_budget WHERE user_id=#{userId} AND month_start=#{monthStart} ORDER BY category_id")
